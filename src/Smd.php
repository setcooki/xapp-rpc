...
abstract class Xapp_Rpc_Smd implements Xapp_Singleton_Interface
{
    ...
    /**
     * option to protect smd view when rpc url is called or viewed without any parameters - smd view. normally this will
     * output/flush smd to screen. if this option is set will prompt http basic auth dialog to enter credentials password
     * and username which are defined in this class option as array with two values, e.g. array('user', 'pass');
     *
     * @const BASIC_AUTH
     */
    const BASIC_AUTH                = 'RPC_SMD_BASIC_AUTH';
    ...
    /**
     * http basic auth if class option BASIC_AUTH is set will prompt login dialog as long as user has entered valid
     * credentials, user and pass. see class option for more details
     *
     * @error 14116
     * @return void
     */
    ...
    {
        if(xapp_has_option(self::BASIC_AUTH, $this))
        {
            $auth = xapp_get_option(self::BASIC_AUTH, $this);
            if(array_key_exists(0, $auth) && array_key_exists(1, $auth))
            {
                if(
                    (!isset($_SERVER['PHP_AUTH_USER']) || !isset($_SERVER['PHP_AUTH_PW']))
                    ||
                    ($_SERVER['PHP_AUTH_USER'] != $auth[0] || $_SERVER['PHP_AUTH_PW'] != $auth[1])
                ){
                    header("WWW-Authenticate: Basic realm=\"xapp rpc smd\"");
                    header("HTTP/1.0 401 Unauthorized");
                    echo "xapp rpc smd view not authorized";
                    exit;
                }
            }
        }
    ...
    {
        return xapp_array_get($this->map(), $name, xapp_default($default));
    }
