        ...
        foreach((array)$params as $p)
        {
            if(is_array($p) && array_key_exists('method', $p) && !empty($p['method']))
            {
                ...
                if(strpos($p['method'], xapp_get_option(self::CLASS_METHOD_SEPARATOR, $this)) !== false)
                {
                    ...
                    $this->_calls[] = array
                    (
                        $this->_services[] = $service . xapp_get_option(self::CLASS_METHOD_SEPARATOR, $this) . $p['method'],
                        $p['method'],
                        $service,
                        $p,
                        $type
    ...
    protected function execute($call)
    {
        ...
        catch(Exception $e)
        {
            if(xapp_get_option(self::COMPLY_TO_JSONRPC_1_2, $this))
            {
                ...
                {
                    xapp_set_option(Xapp_Rpc_Response::STATUS_CODE, $this->codeMap[$e->getCode()], $this->response());
                }else{
                    xapp_set_option(Xapp_Rpc_Response::STATUS_CODE, 500, $this->response());
                }
            }
