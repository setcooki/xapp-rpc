...
abstract class Xapp_Rpc_Request
{
    ...
    {
        return xapp_array_get($this->_params, $name, xapp_default($default));
    }
    ...
    public function getFrom($name, $from = self::PARAMS, $default = null)
    {
        ...
        switch($from)
        {
            case self::PARAMS:
                ...
                break;
            case self::RPC:
                $return = ((isset($GLOBALS['_RPC']) && array_key_exists($name, $GLOBALS['_RPC'])) ? $GLOBALS['_RPC'][$name] : xapp_default($default));
                break;
            case self::POST:
                $return = ((array_key_exists($name, $_POST)) ? $_POST[$name] : xapp_default($default));
                break;
            case self::GET:
                $return = ((array_key_exists($name, $_GET)) ? $_GET[$name] : xapp_default($default));
                break;
            case self::COOKIE:
                $return = ((array_key_exists($name, $_COOKIE)) ? $_COOKIE[$name] : xapp_default($default));
                break;
            case self::SERVER:
