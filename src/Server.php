...
abstract class Xapp_Rpc_Server extends Xapp_Rpc implements Xapp_Singleton_Interface
{
    ...
    /**
     * this option turns on request object validation before passing request parameter to respective function or class.method
     * the validation by default is turned on = boolean true. the xapp internal default validation can be bypassed by
     * passing a php callable as value for this class option. if a callback is set and validation needs to fails must
     * either throw an exception or return false in order for rpc service to stop and return validation errors. see
     * Xapp_Rpc_Server::test for more details
     *
     * @const VALIDATION
     */
    ...
    /**
     * run validation if class option for validation is enabled with boolean true or valid callable. in case validation
     * option is a callable will pass call parameter as first argument and reference to server instance as second argument.
     * NOTE: the call or first argument that callable will receive contains array of requests since request can be batched
     * request so iteration of call argument is required. the actual call argument contains again an array with values at:
     * 0 - service name
     * 1 - function or method name
     * 2 - class name or null
     * 3 - request object as array as original requested
     * 4 - type of request 0 = call, 1 = notification
     * most likely 0 and 3 will be relevant for custom validation. the custom callback in case of negative validation must
     * throw either exception or return boolean false which will trigger a default validation error
     *
     * @error 14226
     * @param array $call expects call array
     * @return void
     * @throws Xapp_Rpc_Fault
     */
    protected function test($call)
    {
        $validation = xapp_get_option(self::VALIDATION, $this);
        ...
        if($validation)
        {
            if(is_bool($validation))
            {
                ...
                $return = call_user_func_array($validation, array($call, &$this));
            }
            if($return === false)
            {
                Xapp_Rpc_Fault::t("custom validation error", array(1422601, -32019));
            }
        }
    }
